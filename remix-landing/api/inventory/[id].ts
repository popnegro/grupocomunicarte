...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'PUT') {
      ...
      return res.status(200).json(DBService.updateSupport(id, body));
    }
    if (req.method === 'DELETE') return DBService.deleteSupport(id) ? res.status(204).end() : res.status(404).json({ error: 'Soporte no encontrado.' });
