...
export function ExplorerPage() {
  ...
  useEffect(() => {
    // Aplicar filtros iniciales cuando el componente se monta
  ...
  const toggleSupportSelection = (support: Support) => {
    setSelectedSupports(prev => 
      prev.some(s => s.id === support.id)
        ? prev.filter(s => s.id !== support.id)
        : [...prev, support]
    );
  };
  ...
  return (
    <>
      {/* Usamos una Navbar simplificada o un botón de volver para la página de exploración */}
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b p-4">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-slate-900">
      ...
      <main className="px-4 sm:px-6 py-6 max-w-7xl mx-auto w-full space-y-5" id="inventory-grid">
        <div className="bg-white border border-[#DCE4DF] p-5 rounded-2xl space-y-4 shadow-2xs">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#DCE4DF] pb-4">
            ...
            <div className="flex items-center space-x-1 p-1 bg-[#F7F9F7] rounded-xl border border-[#DCE4DF] self-start md:self-auto">
              <button onClick={() => setViewMode('map')} className={`px-3.5 py-1.5 rounded-lg text-xs font-extrabold flex items-center gap-1.5 transition-all ${viewMode === 'map' ? 'bg-[#082028] text-white' : 'text-[#40515A]'}`}>
                <MapIcon className="w-3.5 h-3.5" /> Mapa
              </button>
              <button onClick={() => setViewMode('list')} className={`px-3.5 py-1.5 rounded-lg text-xs font-extrabold flex items-center gap-1.5 transition-all ${viewMode === 'list' ? 'bg-[#082028] text-white' : 'text-[#40515A]'}`}>
          ...
          <div className="relative">
            ...
            />
            {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2"><X className="w-4 h-4" /></button>}
          </div>

          <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center space-x-1 bg-[#F7F9F7] p-1 rounded-xl border">
                {(['Todas', 'Mendoza', 'Buenos Aires'] as const).map(p => (
                  <button key={p} onClick={() => setCurrentPlaza(p)} className={`px-3 py-1 rounded-lg text-[11px] font-extrabold ${currentPlaza === p ? 'bg-[#082028] text-white' : 'text-[#40515A]'}`}>{p}</button>
                ))}
              </div>
              <div className="flex flex-wrap items-center gap-1">
                {(['Todos', 'Soportes Tradicionales', 'Pantallas LED', 'LED Móvil'] as const).map(type => (
                  <button key={type} onClick={() => setCurrentType(type)} className={`px-3 py-1 rounded-xl text-[10px] font-extrabold border ${currentType === type ? 'bg-[#7C3AED] text-white' : 'bg-white text-[#40515A]'}`}>{type}</button>
                ))}
              </div>
            </div>
            <button onClick={resetFilters} className="text-xs font-bold text-purple-600 flex items-center gap-1"><RotateCcw className="w-3 h-3" /> Limpiar filtros</button>
          </div>
        </div>
        ...
        : filteredSupports.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            <div className={`lg:col-span-5 ${viewMode === 'list' ? 'lg:col-span-12' : ''}`} style={{ maxHeight: viewMode === 'list' ? 'none' : '600px' }}>
              <div className="max-h-150 overflow-y-auto space-y-2 pr-1">
                {filteredSupports.map(s => {
                  ...
                  return (
                    <div key={s.id} onClick={() => setActiveSupport(s)} className="p-3 rounded-xl border bg-white hover:border-purple-400 cursor-pointer">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex flex-col items-center gap-2">
                           <button onClick={(e) => { e.stopPropagation(); setActiveSupport(s); }} className="p-2 bg-gray-100 rounded-full hover:bg-purple-100"><Eye className="w-4 h-4 text-purple-600" /></button>
                           <button onClick={(e) => { e.stopPropagation(); toggleSupportSelection(s); }} className={`p-2 rounded-full ${isSelected ? 'bg-purple-600 text-white' : 'bg-gray-100'}`}>
            ...
            {viewMode === 'map' && ( // La altura del mapa se gestiona internamente en InteractiveMap
