...
export const SlidesSyncModule: React.FC<SlidesSyncModuleProps> = ({ token, onRefreshInventory }) => {
  ...
  const handleStartSync = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const total = data.data.totalSlides || 0;
        const imp = data.data.importedCount || 0;
        const upd = data.data.updatedCount || 0;
        ...
        toast.error(data.error || "Ocurrió un error en el canal de sincronización.", "Fallo de Importación");
        fetchSyncHistory();
      }
    } catch (err: any) {
  ...
  const handleRollback = async (syncId: number) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `La base de datos se ha revertido con éxito. Se restauraron ${data.data.restoredCount} soportes publicitarios.`,
          "Rollback Exitoso"
        );
        ...
        toast.error(data.error || "No se pudo realizar el rollback.", "Error de Rollback");
      }
    } catch (err: any) {
  ...
  const handleToggleRun = async (runId: number) => {
    ...
    setSelectedRunErrors([]);
    try {
      const res = await fetch(`/api/sync/errors/${runId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
      const data = await res.json();
      if (data.success) {
        setSelectedRunErrors(data.data);
      }
    } catch (err) {
      console.error("Error loading run errors:", err);
    }
  };
