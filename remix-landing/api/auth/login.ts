...
function getPasswordHash(secret: string) {
  return secret.startsWith('pbkdf2_sha256$') ? secret : hashPassword(secret);
}
...
export default function handler(req: any, res: any) {
  ...
  if (email === SUPERADMIN_EMAIL) {
    passwordMatches = verifyPassword(password, getPasswordHash(SUPERADMIN_PASSWORD));
    if (passwordMatches) {
      user = {
        uid: 'sa1',
    ...
    passwordMatches = verifyPassword(password, getPasswordHash(ADMIN_PASSWORD));
    if (passwordMatches) {
      user = {
        uid: 'a1',
        email: ADMIN_EMAIL,
