...
import { Plaza, TipoSoporte, Disponibilidad, InventoryItem, getDisponibilidad } from '../types';
...
function InventarioContent() {
  ...
  const disponibilidadParam = searchParams.get('disponibilidad') as DisponibilidadFilter | null;
  ...
  const [selectedDisponibilidad, setSelectedDisponibilidad] = useState<DisponibilidadFilter>(disponibilidadParam || 'todos');
  ...
  useEffect(() => {
    ...
    if (selectedPlaza !== 'todos') params.set('plaza', selectedPlaza); else params.delete('plaza');
    if (selectedTipo !== 'todos') params.set('tipo', selectedTipo); else params.delete('tipo');
    if (selectedDisponibilidad !== 'todos') params.set('disponibilidad', selectedDisponibilidad); else params.delete('disponibilidad');
    setSearchParams(params, { replace: true });
  }, [selectedPlaza, selectedTipo, selectedDisponibilidad]);
  ...
  useEffect(() => {
    ...
    if (disponibilidadParam && disponibilidadParam !== selectedDisponibilidad) setSelectedDisponibilidad(disponibilidadParam);
  }, [plazaParam, tipoParam, disponibilidadParam]);
  ...
  const matchesDisponibilidad = (item: InventoryItem) => selectedDisponibilidad === 'todos' || getDisponibilidad(item) === selectedDisponibilidad;
  const filteredLocations = fixedLocations.filter((loc) =>
    (selectedPlaza === 'todos' || loc.ciudad === selectedPlaza) &&
    (selectedTipo === 'todos' || loc.tipo_soporte === selectedTipo) && matchesDisponibilidad(loc) && matchesSearch(loc)
  );
  const filteredRoutes = mobileRoutes.filter((route) =>
    (selectedPlaza === 'todos' || route.ciudad === selectedPlaza) &&
    (selectedTipo === 'todos' || route.tipo_soporte === selectedTipo) && matchesDisponibilidad(route) && matchesSearch(route)
  );
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] relative overflow-hidden">
      ...
      <div className={cn('absolute md:relative inset-0 md:inset-auto z-[2000] md:z-10 bg-black/40 md:bg-transparent transition-opacity duration-300 md:opacity-100 md:block', isMobileFiltersOpen ? 'opacity-100 block' : 'opacity-0 hidden')}>
        <div className="absolute md:relative inset-y-0 left-0 w-[85%] max-w-sm md:w-80 h-full bg-white flex flex-col shadow-2xl md:shadow-none border-r border-gray-200">
          ...
          <div className="flex-grow overflow-y-auto">
            <MapFilterPanel selectedPlaza={selectedPlaza} setSelectedPlaza={setSelectedPlaza} selectedTipo={selectedTipo} setSelectedTipo={setSelectedTipo} selectedDisponibilidad={selectedDisponibilidad} setSelectedDisponibilidad={setSelectedDisponibilidad} searchText={searchText} setSearchText={setSearchText} resultsCount={filteredLocations.length + filteredRoutes.length} onOpenMediakit={openMediakit} />
          </div>
        </div>
      </div>
      <div className="flex-grow h-full relative z-0">
        ...
        {isMediakitOpen && <MediakitPanel selectedItems={selectedItems} onClose={closeMediakit} onGoToInventory={closeMediakit} />}
      </div>
    </div>
  );
}

export default function Inventario() { return <InventarioContent />; }
