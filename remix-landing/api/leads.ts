...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      return res.status(200).json(DBService.getLeads());
    }
    if (req.method === 'POST') {
      ...
      return res.status(201).json(DBService.addLead({ name: name.trim(), company: company.trim(), email: email.trim().toLowerCase(), phone: phone.trim(), message: typeof body.message === 'string' ? body.message.trim() : undefined, selectedSupportIds: Array.isArray(body.selectedSupportIds) ? body.selectedSupportIds : [], plazaContext: typeof body.plazaContext === 'string' ? body.plazaContext : undefined, campaignStartDate: typeof body.campaignStartDate === 'string' ? body.campaignStartDate : undefined, campaignEndDate: typeof body.campaignEndDate === 'string' ? body.campaignEndDate : undefined }));
    }
