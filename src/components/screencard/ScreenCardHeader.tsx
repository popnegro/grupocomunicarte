...
export const ScreenCardHeader = React.memo(({ screen, typeStyle, isReserved, availabilityMessage, onFocusOnMap }: ScreenCardHeaderProps) => {
  ...
  return (
    <div className={`relative aspect-video bg-stone-900 flex items-center justify-center text-white overflow-hidden shrink-0 rounded-t-[19px] transition-all duration-300 ${isReserved ? "grayscale saturate-50 opacity-80" : ""}`}>
      {isReserved && (
      ...
      {heroVideo ? (
        <video
          src={heroVideo.url}
          poster={heroMedia?.posterUrl || heroMedia?.url}
          autoPlay
          muted
          loop
          playsInline
        ...
        <img
          src={heroMedia.posterUrl || heroMedia.url}
          alt={screen.nombre}
          className="absolute inset-0 w-full h-full object-cover opacity-90 transition-transform duration-700 ease-out group-hover:scale-103"
          loading="lazy"
