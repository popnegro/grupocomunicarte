...
export function SupportTypePage({ type, setRoute, setInitialFilters }: { type: 'led' | 'tradicional' | 'led-movil', setRoute: (route: string) => void, setInitialFilters: (filters: any) => void }) {
  const details = supportDetails[type];

  const handleExplore = () => {
    setInitialFilters({ type: details.filter });
    setRoute('/explorer');
  };
  ...
  return (
    <div className="relative bg-white">
      <div className="mx-auto max-w-7xl lg:flex lg:justify-between lg:px-8 xl:justify-end">
        <div className="lg:flex-shrink-0 lg:w-1/2 xl:pr-16">
          <div className="px-6 lg:pt-32 lg:pl-0 xl:pl-16">
            <div className="mx-auto max-w-lg lg:mx-0">
              <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">{details.title}</h1>
              <p className="mt-6 text-lg leading-8 text-gray-600">{details.description}</p>
              <div className="mt-10">
                <button onClick={handleExplore} className="rounded-md bg-slate-900 px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-600">
