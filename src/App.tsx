...
import { PmvVisualHome } from "./components/PmvVisualHome";
...
function LoadingScreen({ message }: { message?: string }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, ease: "easeOut" }} className="min-h-screen bg-[#FAF9F5] flex flex-col items-center justify-center p-6 text-center font-sans">
      ...
      <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.25 }} className="text-xs font-bold text-[#06434a] uppercase tracking-widest">{statusMessage}</motion.p>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.25 }} className="mt-2 text-[11px] text-stone-500 max-w-xs leading-relaxed">Por favor espera un momento mientras preparamos el entorno.</motion.p>
    </motion.div>
  );
}
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F5] flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md w-full bg-white border border-stone-200 rounded-3xl p-8 shadow-sm">
          <div className="w-14 h-14 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-center mx-auto mb-4 text-amber-600 font-extrabold text-xl">403</div>
          <h2 className="text-xl font-bold text-stone-900 mb-2">Acceso No Autorizado</h2>
          <p className="text-xs text-stone-600 mb-6">Esta sección requiere privilegios de Administrador. Si eres usuario registrado, contacta al Administrador Principal para habilitar tu rol.</p>
          <button onClick={() => window.location.href = "/dashboard"} className="w-full py-2.5 px-4 bg-[#06434a] hover:bg-[#0a545d] text-white font-bold text-xs rounded-xl shadow-xs transition-colors cursor-pointer">Volver al Panel General</button>
        </div>
      </div>
    );
  }
    ...
    <LazyAuthProvider>
      <ToastProvider>
        <BrowserRouter>
          <CmsProvider>
            <Routes>
              <Route path="/" element={<PmvVisualHome />} />
