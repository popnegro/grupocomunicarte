...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') return res.status(200).json(DBService.getSupports());
    ...
    if (req.method === 'POST') {
      ...
      return res.status(201).json(DBService.addSupport({ ...body, name: String(body.name).trim(), address: String(body.address).trim(), description: String(body.description).trim(), size: String(body.size).trim(), imageUrl: String(body.imageUrl).trim(), status: body.status === 'reserved' ? 'reserved' : 'available' }));
    }
