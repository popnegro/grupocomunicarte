...
export class DBService {
  ...
  public static async addMediaKit(mediaKit:Omit<MediaKit,'id'|'createdAt'>): Promise<MediaKit> { const db=requireDb(); await ensureSchema(); const item={...mediaKit,id:`mediakit-${crypto.randomUUID()}`,createdAt:new Date().toISOString()}; await db`INSERT INTO mediakits (id,data) VALUES (${item.id},${JSON.stringify(item)}::jsonb)`; return item; }
  public static async deleteMediaKit(id:string): Promise<boolean> { const db=requireDb(); await ensureSchema(); const result=await db`DELETE FROM mediakits WHERE id=${id}`; return result.count>0; }
}
