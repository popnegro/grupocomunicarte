...
export const MediaKitModule: React.FC<MediaKitModuleProps> = ({
  ...
  const handleExportToGoogleSlides = async (e?: React.FormEvent, forceToken?: string) => {
    ...
    try {
      ...
      let res = await safeFetchJson<{ success: boolean; needsAuth?: boolean; presentationUrl?: string; error?: string }>(
        `/api/mediakits/${activeMediaKit.id}/export-slides`,
        {
          method: "POST",
          headers: {
      ...
      if (res.status === 401 || res.data?.needsAuth) {
        ...
        if (urlRes.data?.success && urlRes.data?.url) {
          ...
          const timer = setInterval(async () => {
            if (!authWindow || authWindow.closed) {
              ...
              let retryRes = await safeFetchJson<{ success: boolean; presentationUrl?: string }>(
                `/api/mediakits/${activeMediaKit.id}/export-slides`,
                {
                  method: "POST",
                  headers: {
              ...
              if (retryRes.data?.success && retryRes.data.presentationUrl) {
                setSlidesExportResult({ url: retryRes.data.presentationUrl });
              ...
              const { googleSlidesAuthProvider, googleAuthProvider, signInWithPopup, auth } = await import("../../lib/firebase");
              let loginRes;
              try {
                loginRes = await signInWithPopup(auth, googleSlidesAuthProvider);
              } catch {
                loginRes = await signInWithPopup(auth, googleAuthProvider);
              }
          ...
          if (credential?.accessToken) {
            ...
            res = await safeFetchJson<{ success: boolean; presentationUrl?: string; error?: string }>(
              `/api/mediakits/${activeMediaKit.id}/export-slides`,
              {
                method: "POST",
                headers: {
      ...
      if (res.data?.success && res.data.presentationUrl) {
        setSlidesExportResult({ url: res.data.presentationUrl });
        triggerToast("¡Presentación de Google Slides exportada con éxito!");
      } else {
        throw new Error(res.data?.error || res.error || "Error al exportar a Google Slides.");
      }
    } catch (error: any) {
