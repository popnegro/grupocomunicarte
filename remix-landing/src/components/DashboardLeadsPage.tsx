  ...
  return (
    <section className="space-y-5">
      <header>
        ...
        <h1 className="mt-1 text-2xl font-extrabold">Leads</h1>
        <p className="mt-1 text-sm text-[#64748B]">Gestioná las solicitudes desde el primer contacto hasta el archivo.</p>
      </header>
        ...
        {columns.map((column) => (
          ...
          >
            <div className="mb-3 flex items-center justify-between px-2 py-1">
              <div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${column.key === 'pending' ? 'bg-amber-400' : column.key === 'contacted' ? 'bg-sky-500' : 'bg-slate-400'}`} /><h2 className="text-sm font-extrabold">{column.label}</h2></div>
            ...
            <div className="space-y-3">
              ...
              {grouped[column.key].length === 0 && <div className="rounded-xl border border-dashed border-[#DCE4DF] px-4 py-10 text-center text-xs font-semibold text-[#94A3B8]">Arrastrá leads aquí</div>}
            </div>
          </div>
        ))}
      </div>
