import { fixedLocations } from '../data/inventory';
...
const inventory: InventoryItem[] = fixedLocations;
...
export function listInventory(filters: InventoryFilters = {}): InventoryItem[] {
  ...
  return inventory.filter((item) => {
    ...
    return (!query || haystack.includes(query)) && (availability === 'todos' || getDisponibilidad(item) === availability) && (plaza === 'todas' || item.ciudad === plaza);
  });
}
