...
import { X, MapPin, CheckCircle2, Image as ImageIcon } from 'lucide-react';
...
export function MediakitPanel({ selectedItems, onClose, onGoToInventory }: MediakitPanelProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    // No hay backend de generación de Mediakit en el PMV: se valida y se confirma la solicitud en la UI.
    setSubmitted(true);
  };
        ...
        {submitted ? (
          <div className="flex flex-col items-center text-center py-6">
            ...
            <p className="text-sm text-gray-500 mb-8 max-w-xs">
              Recibimos tu solicitud de Mediakit para {selectedItems.length} {selectedItems.length === 1 ? 'soporte' : 'soportes'}. Te lo enviamos por email a la brevedad.
            </p>
          ...
          <div className="flex flex-col items-center text-center py-6">
            ...
            <p className="text-sm text-gray-500 mb-8 max-w-xs">
              Elegí uno o más soportes disponibles en el mapa para solicitar su Mediakit.
            </p>
          ...
          <>
            <div className="mb-6">
              <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">
                Soportes seleccionados ({selectedItems.length})
              </h4>
                ...
                {selectedItems.map((item) => (
                  <div key={item.canonical_id} className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-lg p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Badge variant="neutral" className="mt-1 uppercase tracking-wider">
                        {isMobileRoute(item) ? 'led movil' : item.tipo_soporte.replace('_', ' ')}
                      </Badge>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeSelected(item.canonical_id)}
                      className="p-1.5 text-gray-400 hover:text-black hover:bg-gray-100 rounded-full transition-colors shrink-0"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Label htmlFor="mk-name">Nombre</Label>
                <Input id="mk-name" name="name" required placeholder="Tu nombre" />
              </div>
              <div>
                <Label htmlFor="mk-company">Empresa</Label>
                <Input id="mk-company" name="company" placeholder="Nombre de tu empresa (opcional)" />
              </div>
              <div>
                <Label htmlFor="mk-email">Email</Label>
                <Input id="mk-email" name="email" type="email" required placeholder="tu@empresa.com" />
              </div>
              <div>
                <Label htmlFor="mk-phone">Teléfono</Label>
                <Input id="mk-phone" name="phone" type="tel" placeholder="Opcional" />
              </div>
              <div>
                <Label htmlFor="mk-message">Observaciones</Label>
                <Textarea id="mk-message" name="message" rows={3} placeholder="Contanos sobre tu campaña (opcional)" />
              </div>
              <Button type="submit" className="w-full">
                Solicitar Mediakit ({selectedItems.length})
              </Button>
            </form>
          </>
        )}
