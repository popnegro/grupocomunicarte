      ...
      <div className="relative mb-6">
        ...
        {searchText && (
          <button
            ...
            className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 text-gray-400 hover:text-black rounded-full hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-1"
          >
      ...
      <div className="space-y-8 overflow-y-auto">
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-2">
            <FilterButton active={selectedPlaza === 'todos'} onClick={() => setSelectedPlaza('todos')} label="Todas las Plazas" icon={<MapPin className="w-4 h-4" aria-hidden="true" />} />
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-2">
            <FilterButton active={selectedTipo === 'todos'} onClick={() => setSelectedTipo('todos')} label="Todos los Soportes" icon={<AlignLeft className="w-4 h-4" aria-hidden="true" />} />
            <FilterButton active={selectedTipo === 'tradicional'} onClick={() => setSelectedTipo('tradicional')} label="Tradicionales" icon={<PanelTop className="w-4 h-4" aria-hidden="true" />} />
            <FilterButton active={selectedTipo === 'led'} onClick={() => setSelectedTipo('led')} label="Pantallas LED" icon={<MonitorPlay className="w-4 h-4" aria-hidden="true" />} />
            <FilterButton active={selectedTipo === 'led_movil'} onClick={() => setSelectedTipo('led_movil')} label="LED Móvil" icon={<MonitorPlay className="w-4 h-4" aria-hidden="true" />} />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            <FilterButton active={selectedDisponibilidad === 'disponible'} onClick={() => setSelectedDisponibilidad('disponible')} label="Disponibles" icon={<CheckCircle2 className="w-4 h-4" aria-hidden="true" />} />
            <FilterButton active={selectedDisponibilidad === 'reservado'} onClick={() => setSelectedDisponibilidad('reservado')} label="Reservados" icon={<Lock className="w-4 h-4" aria-hidden="true" />} />
          </div>
        </div>
      </div>

      <div className="mt-auto pt-8 space-y-3">
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 flex items-center justify-between" aria-live="polite">
...
function FilterButton({ active, onClick, label, icon }: { active: boolean, onClick: () => void, label: string, icon?: ReactNode }) {
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2",
        active
    ...
    >
      ...
      {!icon && <span className="w-4" aria-hidden="true"></span>}
      {label}
    </button>
  );
}
